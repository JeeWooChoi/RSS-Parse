...
export async function syncPodcastFromExcel({
  ...
  if (existingProgram) {
    ...
    if (currentCount >= EPISODE_LIMIT) {
      ...
      // 테마 매핑 진행
      await syncThemeMapping(existingProgram.id, orderPopular, programTitle);
  ...
  await syncCategoryMapping(finalProgram.id, categoryId, country, programTitle);

  /* ---------------- 테마 프로그램 매핑 (themes_programs) ---------------- */

  await syncThemeMapping(finalProgram.id, orderPopular, programTitle);
